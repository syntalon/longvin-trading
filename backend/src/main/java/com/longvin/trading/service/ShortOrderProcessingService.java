...
public class ShortOrderProcessingService {
    ...
    public void processLocateResponse(UUID locateRequestId,
        ...
        try {
            ...
            // Get shadow accounts from database
            List<Account> shadowAccounts = accountRepository.findByAccountTypeAndActiveTrue(AccountType.SHADOW);
            
            if (shadowAccounts.isEmpty()) {
                log.warn("No active shadow accounts found, cannot place shadow orders");
                return;
            }

            // Place orders for shadow accounts
            if (locateRequest.getStatus() == LocateRequest.LocateStatus.BORROWED) {
                placeShadowOrders(locateRequest.getOrder(), shadowAccounts, initiatorSessionID);
            } else {
                log.warn("Cannot place shadow orders: stock not borrowed (status: {})", locateRequest.getStatus());
            }
        } catch (Exception e) {
    ...
    private void borrowStock(LocateRequest locateRequest) {
        ...
        // In a real system, this would call broker API to actually borrow the stock
    ...
    /**
     * Place orders for shadow accounts after stock is borrowed.
     */
    private void placeShadowOrders(Order primaryOrder, java.util.List<Account> shadowAccounts, SessionID initiatorSessionID) {
        log.info("Placing shadow orders for {} shadow accounts", shadowAccounts.size());

        if (shadowAccounts.isEmpty()) {
            log.warn("No shadow accounts available for order {}", primaryOrder.getId());
            return;
        }
        ...
        for (Account shadowAccount : shadowAccounts) {
            try {
                NewOrderSingle shadowOrder = buildShadowOrder(primaryOrder, shadowAccount);
                Session.sendToTarget(shadowOrder, initiatorSessionID);
                successCount++;
                log.info("Placed shadow order for account {}: Symbol={}, Side={}, Qty={}, ClOrdID={}", 
                    shadowAccount.getAccountNumber(), 
                    primaryOrder.getSymbol(), 
                    primaryOrder.getSide(), 
                    primaryOrder.getOrderQty(),
                    shadowOrder.getString(ClOrdID.FIELD));
            } catch (SessionNotFound e) {
                failureCount++;
                log.error("Session not found when placing shadow order for account {}: {}", 
                    shadowAccount.getAccountNumber(), e.getMessage());
            } catch (FieldNotFound e) {
                failureCount++;
                log.error("Missing required field when placing shadow order for account {}: {}", 
                    shadowAccount.getAccountNumber(), e.getMessage());
            } catch (Exception e) {
                failureCount++;
                log.error("Error placing shadow order for account {}: {}", 
                    shadowAccount.getAccountNumber(), e.getMessage(), e);
            }
        }

        log.info("Shadow order placement completed: {} succeeded, {} failed out of {} total", 
            successCount, failureCount, shadowAccounts.size());
    }

    /**
     * Build a NewOrderSingle for a shadow account based on the primary order.
     */
    private NewOrderSingle buildShadowOrder(Order primaryOrder, Account shadowAccount) throws FieldNotFound {
        NewOrderSingle order = new NewOrderSingle();
        
        // Generate unique ClOrdID for shadow order
        String clOrdId = "SHADOW-" + primaryOrder.getFixClOrdId() + "-" + shadowAccount.getAccountNumber();
        ...
        order.setString(Symbol.FIELD, primaryOrder.getSymbol());
        order.set(new Side(primaryOrder.getSide()));
        order.set(new TransactTime());
        
        if (primaryOrder.getOrdType() != null) {
            order.set(new OrdType(primaryOrder.getOrdType()));
        }
        if (primaryOrder.getOrderQty() != null) {
            order.set(new OrderQty(primaryOrder.getOrderQty().doubleValue()));
        }
        if (primaryOrder.getPrice() != null) {
            order.set(new Price(primaryOrder.getPrice().doubleValue()));
        }
        if (primaryOrder.getStopPx() != null) {
            order.set(new StopPx(primaryOrder.getStopPx().doubleValue()));
        }
        if (primaryOrder.getTimeInForce() != null) {
            order.set(new TimeInForce(primaryOrder.getTimeInForce()));
        }
        
        // Set shadow account
        order.setString(quickfix.field.Account.FIELD, shadowAccount.getAccountNumber());
